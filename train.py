...
parser.add_argument('--output_path', type=str, default='/media/tai/6TB/Projects/InfoMUNIT/Models/MUNIT-reremake/MUNIT_DD_init', help="outputs path")
...
while True:
    for it, (samples_a, samples_b, samples_a_limited) in enumerate(
        ...
        with Timer("Elapsed time in update: %f"):
            ...
            trainer.cla_digit_update(images_a, [images_a_limited, labels_a_limited], [images_b, labels_b])
            # time_con_cla = time()
            # print(f'Cla: {time_con_cla - time_gen}')

            trainer.cla_domain_update(images_a, images_b)
